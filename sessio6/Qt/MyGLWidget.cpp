...
void MyGLWidget::modelTransform () {
  ...
  glm::vec3  posicio(1,0.5*scale ,1);
...
void MyGLWidget::resizeGL (int w, int h) {
    if(perspective){
        ...
        ra = raV;
        if(raV < 1.0) FOV = 2.0*atan((float) M_PI/4.0/raV);
        projectTransform();
    }
    else {
        // Versio amb l'esfera.
...
void MyGLWidget::carregaShaders() {
  ...
  program->addShader(&vs); // escalar a més gran
...
void MyGLWidget::modelLegoMan(){
    glm::mat4 transform (1.0f);
    glm::vec3  posicio(1,0.5*scale,1);
    transform = glm::translate(transform, posicio);

    float escalat = 1.0/(legoMaxim.y - legoMinim.y);
    transform = glm::scale(transform, glm::vec3(escalat, escalat, escalat));
    transform = glm::scale(transform, glm::vec3(scale));
...
void MyGLWidget::carregaLego(){
    glGenVertexArrays(1, &VAOlego);
    glBindVertexArray(VAOlego);

    glGenBuffers(1, &VBOlegoVertex);
    glBindBuffer(GL_ARRAY_BUFFER, VBOlegoVertex);
    ...
    glGenBuffers(1, &VBOlegoMaterial);
    glBindBuffer(GL_ARRAY_BUFFER, VBOlegoMaterial);
...
void MyGLWidget::projectTransform(){
    glm::mat4 Proj;
    if(perspective) Proj = glm::perspective (FOV, ra, zNear, zFar); // <-- Sessio 4, exercici 2.
    //left = -2.0;
    //right = 2.0;
    //top = 0.5;
    //bottom = -0.5;
...
void MyGLWidget::calculaCapsaLego(){
    // Inicialització del punt mínim.
    legoMinim.x = lego.vertices()[0];
    legoMinim.y = lego.vertices()[1];
    legoMinim.z = lego.vertices()[2];

    // Inicialització del punt màxim.
    legoMaxim.x = lego.vertices()[0];
    legoMaxim.y = lego.vertices()[1];
    legoMaxim.z = lego.vertices()[2];

    int mida = lego.vertices().size();

    for(int i = 3; i < mida; i+=3){
        float auxX = lego.vertices()[i];
        float auxY = lego.vertices()[i+1];
        float auxZ = lego.vertices()[i+2];

        legoMinim.x = std::min(legoMinim.x, auxX);
        legoMaxim.x = std::max(legoMaxim.x, auxX);

        legoMinim.y = std::min(legoMinim.y, auxY);
        legoMaxim.y = std::max(legoMaxim.y, auxY);

        legoMinim.z = std::min(legoMinim.z, auxZ);
        legoMaxim.z = std::max(legoMaxim.z, auxZ);
    }

    centrelego = glm::vec3((legoMaxim.x + legoMinim.x)/2, (legoMaxim.y + legoMinim.y)/2, (legoMaxim.z + legoMinim.z)/2);
